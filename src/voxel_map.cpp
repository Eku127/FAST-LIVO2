...
void loadVoxelConfig(rclcpp::Node* node, VoxelMapConfig &voxel_config)
{
  ...
  node->declare_parameter<double>("lio.sigma_num", 3.0);
  ...
  std::vector<int64_t> layer_init_num_int64;
  node->get_parameter("lio.layer_init_num", layer_init_num_int64);
  voxel_config.layer_init_num_.clear();
  for (auto val : layer_init_num_int64) {
  ...
  std::cout << "VoxelMapConfig: " << std::endl;
  std::cout << "  max_voxel_size_: " << voxel_config.max_voxel_size_ << std::endl;
  std::cout << "  map_sliding_en_: " << voxel_config.map_sliding_en << std::endl;
  std::cout << "  half_map_size_: " << voxel_config.half_map_size << std::endl;
}
...
void VoxelOctoTree::cut_octo_tree()
{
  ...
  for (size_t i = 0; i < temp_points_.size(); i++)
  {
    ...
    {
      leaves_[leafnum] = std::make_unique<VoxelOctoTree>(max_layer_, layer_ + 1, layer_init_num_[layer_ + 1], max_points_num_, planer_threshold_);
...
{
  static int frame_print_counter = 0;
  const int print_interval = 10;  // Print every 10 frames
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    // Only print on last iteration and every N frames to reduce output
    if (iterCount == config_setting_.max_iterations_ - 1 && (frame_print_counter % print_interval == 0))
    {
  ...
  // Increment frame counter for print throttling
  frame_print_counter++;
}
...
void VoxelMapManager::BuildVoxelMap()
{
  ...
  for (uint i = 0; i < plsize; i++)
  {
    ...
    {
      auto octo_tree = std::make_shared<VoxelOctoTree>(max_layer, 0, layer_init_num[0], max_points_num, planer_threshold);
  ...
  for (size_t i = 0; i < index.size(); i++)
  {
    ...
    if (iter != voxel_map_.end())
    {
      ...
      if (!is_sucess)
      {
        ...
        if (iter_near != voxel_map_.end()) { build_single_residual(pv, iter_near->second.get(), 0, is_sucess, prob, single_ptpl); }
      }
      if (is_sucess)
      {
        mylock.lock();
    ...
    if (current_layer < max_layer)
    {
      for (size_t leafnum = 0; leafnum < 8; leafnum++)
      {
        if (current_octo->leaves_[leafnum] != nullptr)
        {
          ...
          VoxelOctoTree *leaf_octo = current_octo->leaves_[leafnum].get();
...
void VoxelMapManager::pubVoxelMap()
{
  ...
  for (auto iter = voxel_map_.begin(); iter != voxel_map_.end(); iter++)
  {
    GetUpdatePlane(iter->second.get(), config_setting_.max_layer_, pub_plane_list);
  }
...
void VoxelMapManager::GetUpdatePlane(const VoxelOctoTree *current_octo, const int pub_max_voxel_layer, std::vector<VoxelPlane> &plane_list)
{
  ...
  if (current_octo->layer_ < current_octo->max_layer_)
  {
    if (!current_octo->plane_ptr_->is_plane_)
    {
      ...
      {
        if (current_octo->leaves_[i] != nullptr) { GetUpdatePlane(current_octo->leaves_[i].get(), pub_max_voxel_layer, plane_list); }
      }
    }
  }
  return;
...
void VoxelMapManager::clearMemOutOfMap(const int& x_max,const int& x_min,const int& y_max,const int& y_min,const int& z_max,const int& z_min )
{
  ...
  for (auto it = voxel_map_.begin(); it != voxel_map_.end(); )
  {
    ...
    if (should_remove){
      ...
      // shared_ptr automatically releases memory when erased
