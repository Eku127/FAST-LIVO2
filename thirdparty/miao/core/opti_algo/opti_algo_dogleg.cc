...
OptimizationAlgorithm::SolverResult OptimizationAlgorithmDogleg::Solve(int iteration) {
    ...
    solver_->BuildSystem();
    Eigen::Map<const lightning::VectorX> b(solver_->GetB(), solver_->VectorSize());
    ...
    do {
        ...
        if (!solvedGaussNewton) {
            ...
            hgnNorm = Eigen::Map<const lightning::VectorX>(solver_->GetX(), solver_->VectorSize()).norm();
        }

        Eigen::Map<const lightning::VectorX> hgn(solver_->GetX(), solver_->VectorSize());
